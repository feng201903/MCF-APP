<template>
  <div>
    <mt-header fixed :title="$t('title.search')">
    ...
    <div class="asset-list" ref="cityList">
      <mt-index-list>
        <mt-index-section v-for="(item,index) in assetsListSort" :key="index" :index="item.assetTp">
          <mt-cell v-for="(list,indexList) in item.data" :key="indexList" :title="list.name" @click.native="$router.push({ path: '/deal', query: { assetid: list.assetId}})">
            <img :src="list.logo">
  ...
  methods: {
    getAssetsList () {
      ...
      if (allAsset.length <= 0) {
        ...
        this.assetsList = allAsset.filter(item => {
